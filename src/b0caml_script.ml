...
type t =
  { file : Filepath.t;
    ...
    directories : (Filepath.t * smeta) list;
    mod_uses : (Filepath.t * smeta) list;
...
let pp_dump ppf s =
  ...
  let pp_paths = Fmt.(list ~sep:sp (pp_fst Filepath.pp_quoted)) in
  Fmt.record
...
| 0x22 ->
    ...
    | 0x22 ->
        ...
        let fpath = match Filepath.of_string (Textdec.lexeme_pop d) with
        | Ok fpath -> fpath
        | Error e -> err loc e
        in
...
type directory_resolution_error = Filepath.t * smeta * [`Error of string | `Miss ]

let directory_resolution_dir (dir, _) = Filepath.ensure_trailing_dir_sep dir

let resolve_directory ~ocamlpath script_root ~errs ~dirs (d, m) =
  match B0caml_ocamlpath.classify_path d with
  | `Concrete dir ->
      let dir = Filepath.(script_root // dir) in
  ...
  | `Logical dir ->
      ...
      let ds = List.map (fun r -> Filepath.append r dir) ocamlpath in
...
let resolve_directories ~ocamlpath s =
  ...
  in
  loop ~ocamlpath (Filepath.parent (file s)) [] [] (directories s)
...
let resolve_mod_use script_root (mod_use, m) =
  let impl_file = Filepath.(script_root // mod_use) in
  match Os.File.exists impl_file with
  | Error e -> Result.Error (impl_file, m, `Error e)
  | Ok false -> Error (impl_file, m, `Miss)
  | Ok true  ->
      if not (Filepath.has_ext ".ml" impl_file)
      then Ok (None, impl_file, m) else
      let intf_file = Filepath.with_ext ~multi:false ".mli" impl_file in
...
let resolve_mod_uses s =
  ...
  in
  loop (Filepath.parent (file s)) [] [] (mod_uses s)
...
let src ~mod_use_resolutions s =
  ...
  let add_dep_src acc (intf_file, impl_file, meta) =
    let name = Filepath.basename impl_file in
  ...
  let add_ocaml_unit s l =
    ...
    src :: "\"\n" :: Filepath.to_string (file s) :: " \"" :: line :: "#" :: l
  in
  try
